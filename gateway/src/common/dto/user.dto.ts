...
import { IGroupMembership, IUser } from '../types';

export class GroupMembershipDto implements IGroupMembership {
  @ApiProperty({
    description: 'It matches \'group\' from collection \'groups\' from DB. Valid MongoDB compatible ObjectId',
    required: true,
...
export class UserDto implements IUser {
  ...
  @ApiProperty({
    description: 'Array of pairs of roles and group ids.',
    required: true,
    uniqueItems: true,
    example: [ { group: '62a588187cebf9ce17bea893', role: ROLE.USER }, { group: '62a826ad1774f165f826923f', role: ROLE.ADMIN } ],
  })
  @IsArray({ message: 'Groups must be an array' })
  @ValidateNested({ each: true })
  @ArrayMaxSize(GROUPS_MAX_SIZE)
