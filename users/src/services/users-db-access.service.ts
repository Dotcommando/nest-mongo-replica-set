...
import { getUserUpdates, mapUserDocumentToIUser } from '../helpers';
import { IUpdateUserReq, IUserDocument } from '../types';
...
export class UsersDbAccessService {
  ...
  public async updateUser(userUpdates: IUpdateUserReq): Promise<{ user: IUser }> {
    const { user, session } = userUpdates;

    try {
      const oldUserDoc: IUserDocument = await this.userModel.findOne({ _id: user._id });
      const userDoc: IUserDocument = await this.userModel.findOneAndUpdate(
        { _id: user._id },
        getUserUpdates(user, oldUserDoc),
        session ? { session, new: true } : { new: true },
      );

      return { user: mapUserDocumentToIUser(userDoc) };
    } catch (e) {
      return { user: null };
    }
  }
